...
当前采用显式接收：`message` 派发，`wait` 接收，AI 检查结果后决定下一步。runtime 不自动续跑父会话。
Workflow 先派发所有独立工作线，再逐组或并行调用 `wait`；组内接力由成员 `message` 登记后继，
管理者只接收整个组的最后结果。外层 parent 接收管理者，不越级接收管理者拥有的组。
