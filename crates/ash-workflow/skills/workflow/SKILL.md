...
For two parallel groups: message the first member of each group, receive both submission receipts, then wait once per group_id. The two waits may run together. Before your final reply, list your work lines to check that none you started is running or unread. Do not ask the outer parent to collect groups it does not own.
