...
Different groups and independent agents may run concurrently. A group is strictly serial. Send work to every independent line you intend to run. Completion results arrive automatically after each line stops. Use history(group_id=...) to inspect shared communication, with before for older pages.

An accepted message is not a completed task. Wait for the runtime completion notice before sending dependent work. A message to running work cancels that work and starts the accepted replacement after cancellation settles. If another replacement is already settling, retry later. Cancellation does not undo edits. Failures and help requests arrive as final results; inspect them and decide the next message.
...
For two parallel groups: message the first member of each group, then process the two completion notices as they arrive. Before your final reply, list your work lines to check that none you started is running. Do not ask the outer parent to collect groups it does not own.
